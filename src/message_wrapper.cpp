...
const sbg_driver::SbgImuStatus MessageWrapper::createImuStatusMessage(uint16_t sbg_imu_status) const
{
  ...
  if (m_use_enu_)
  {
    imu_status_message.imu_accel_x          = (sbg_imu_status & SBG_ECOM_IMU_ACCEL_Y_BIT) != 0;
    imu_status_message.imu_accel_y          = (sbg_imu_status & SBG_ECOM_IMU_ACCEL_X_BIT) != 0;
...
const sbg_driver::SbgMagStatus MessageWrapper::createMagStatusMessage(const SbgLogMag& ref_log_mag) const
{
  ...
  if (m_use_enu_)
  {
    mag_status_message.mag_x            = (ref_log_mag.status & SBG_ECOM_MAG_MAG_Y_BIT) != 0;
    mag_status_message.mag_y            = (ref_log_mag.status & SBG_ECOM_MAG_MAG_X_BIT) != 0;
    mag_status_message.mag_z            = (ref_log_mag.status & SBG_ECOM_MAG_MAG_Z_BIT) != 0;
    mag_status_message.accel_x          = (ref_log_mag.status & SBG_ECOM_MAG_ACCEL_Y_BIT) != 0;
    mag_status_message.accel_y          = (ref_log_mag.status & SBG_ECOM_MAG_ACCEL_X_BIT) != 0;
...
const sbg_driver::SbgEkfNav MessageWrapper::createSbgEkfNavMessage(const SbgLogEkfNavData& ref_log_ekf_nav) const
{
  ...
  if (m_use_enu_)
  {
    ekf_nav_message.velocity          = createVector3Enu<float>(ref_log_ekf_nav.velocity, 3);
    ekf_nav_message.velocity_accuracy = createVector3Enu<float>(ref_log_ekf_nav.velocityStdDev, 3);
    ekf_nav_message.position          = createVector3Enu<double>(ref_log_ekf_nav.position, 3);
  }
  else
  {
    ekf_nav_message.velocity          = createVector3<float>(ref_log_ekf_nav.velocity, 3);
    ekf_nav_message.velocity_accuracy = createVector3<float>(ref_log_ekf_nav.velocityStdDev, 3);
    ekf_nav_message.position          = createVector3<double>(ref_log_ekf_nav.position, 3);
  }
...
const sbg_driver::SbgGpsPos MessageWrapper::createSbgGpsPosMessage(const SbgLogGpsPos& ref_log_gps_pos) const
{
  ...
  if (m_use_enu_)
  {
    gps_pos_message.position.x          = ref_log_gps_pos.longitude;
    gps_pos_message.position.y          = ref_log_gps_pos.latitude;
...
const sbg_driver::SbgGpsVel MessageWrapper::createSbgGpsVelMessage(const SbgLogGpsVel& ref_log_gps_vel) const
{
  ...
  {
    gps_vel_message.vel     = createVector3<float>(ref_log_gps_vel.velocity, 3);
    gps_vel_message.vel_acc = createVector3<float>(ref_log_gps_vel.velocityAcc, 3);
...
const sbg_driver::SbgImuData MessageWrapper::createSbgImuDataMessage(const SbgLogImuData& ref_log_imu_data) const
{
  ...
  if (m_use_enu_)
  {
    imu_data_message.accel        = createVector3Enu<float>(ref_log_imu_data.accelerometers, 3);
    imu_data_message.gyro         = createVector3Enu<float>(ref_log_imu_data.gyroscopes, 3);
    imu_data_message.delta_vel    = createVector3Enu<float>(ref_log_imu_data.deltaVelocity, 3);
    imu_data_message.delta_angle  = createVector3Enu<float>(ref_log_imu_data.deltaAngle, 3);
  }
  else
  {
    imu_data_message.accel        = createVector3<float>(ref_log_imu_data.accelerometers, 3);
    imu_data_message.gyro         = createVector3<float>(ref_log_imu_data.gyroscopes, 3);
    imu_data_message.delta_vel    = createVector3<float>(ref_log_imu_data.deltaVelocity, 3);
    imu_data_message.delta_angle  = createVector3<float>(ref_log_imu_data.deltaAngle, 3);
  }
...
const sbg_driver::SbgMag MessageWrapper::createSbgMagMessage(const SbgLogMag& ref_log_mag) const
{
  ...
  if (m_use_enu_)
  {
    mag_message.mag   = createVector3Enu<float>(ref_log_mag.magnetometers, 3);
    mag_message.accel = createVector3Enu<float>(ref_log_mag.accelerometers, 3);
  }
  else
  {
    mag_message.mag   = createVector3<float>(ref_log_mag.magnetometers, 3);
    mag_message.accel = createVector3<float>(ref_log_mag.accelerometers, 3);
  }
...
const sbg_driver::SbgShipMotion MessageWrapper::createSbgShipMotionMessage(const SbgLogShipMotionData& ref_log_ship_motion) const
{
  ...
  ship_motion_message.ship_motion   = createVector3<float>(ref_log_ship_motion.shipMotion, 3);
  ship_motion_message.acceleration  = createVector3<float>(ref_log_ship_motion.shipAccel, 3);
  ship_motion_message.velocity      = createVector3<float>(ref_log_ship_motion.shipVel, 3);
...
const sbg_driver::SbgImuShort MessageWrapper::createSbgImuShortMessage(const SbgLogImuShort& ref_short_imu_log) const
{
  ...
  if (m_use_enu_)
  {
    imu_short_message.delta_velocity  = createVector3Enu<int32_t>(ref_short_imu_log.deltaVelocity, 3);
    imu_short_message.delta_angle     = createVector3Enu<int32_t>(ref_short_imu_log.deltaAngle, 3);
  }
  else
  {
    imu_short_message.delta_velocity  = createVector3<int32_t>(ref_short_imu_log.deltaVelocity, 3);
    imu_short_message.delta_angle     = createVector3<int32_t>(ref_short_imu_log.deltaAngle, 3);
  }
...
const sensor_msgs::NavSatFix MessageWrapper::createRosNavSatFixMessage(const sbg_driver::SbgGpsPos& ref_sbg_gps_msg) const
{
  ...
  if (m_use_enu_)
  {
    nav_sat_fix_message.latitude  = ref_sbg_gps_msg.position.y;
    nav_sat_fix_message.longitude = ref_sbg_gps_msg.position.x;
    nav_sat_fix_message.altitude  = ref_sbg_gps_msg.position.z;

    nav_sat_fix_message.position_covariance[0] = pow(ref_sbg_gps_msg.position_accuracy.y, 2);
    nav_sat_fix_message.position_covariance[4] = pow(ref_sbg_gps_msg.position_accuracy.x, 2);
